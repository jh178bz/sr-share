...
RSpec.describe Item, type: :model do
  ...
  it "is invalid without a duplicate name" do
    create(:item, name: "テストタイヤ")
    expect(build(:item, name: "テストタイヤ")).to_not be_valid
  end
  # 名前が30以上は無効
  it "is invalid 30 characters or more a name" do
    expect(build(:item, name: "a" * 30 + "name")).to_not be_valid
  end
  # content無ければ無効
  ...
  it "is invalid 300 characters or more a content" do
    expect(build(:item, content: "a" * 300 + "comment")).to_not be_valid
  end
  # 画像なしでも有効
